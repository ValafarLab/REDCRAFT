...
//Matrix* augmentMatrix(Matrix *a);
Matrix* matrixFromArray(int row_size, int col_size, double** M);
/**
...
public:
    static string svn_id() { return "$Id: matrix.h 418 2012-10-25 20:48:10Z siminm $"; }
    ...
    void Set_Mij(int i, int j, double x) { M[i][j] = x; }
    double Get_Mij(int i, int j) const { return M[i][j]; }
    int GetRow() const { return row; }
    int GetCol() const { return col; }

    const Matrix & operator= (const Matrix &);
    const Matrix & operator= (const Eigen::MatrixXd &);
