...
Matrix* invertSquareMatrix(Matrix *a) {
    int N = a->GetCol();
    if (N < 2) return a;
    // use 1 dimensional arrays in order to maximize GPU performance
    double I[N*N];
    double A[N*N];
...
#define A(i,j) (A[((i)*N) + (j)])
    ...
    // copy contents of matrix parameter into 1D array for GPU offloading
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            A(i,j) = a->M[i][j];
        }
...
//            printf("Num Threads in use:%d ",omp_get_num_threads());
...
#pragma omp parallel for
    ...
    for (int zeroing_col = N - 1; zeroing_col >= 1; zeroing_col--) {
#pragma omp parallel for
//#pragma omp target data  map (tofrom: A[0:N*N], I[0:N*N])
//#pragma omp target
//#pragma omp teams distribute parallel for schedule(static, 1)
        for (int row = zeroing_col - 1; row >= 0; row--) {
            double factor = A(row,zeroing_col);
            for (int col = 0; col < N; col++) {
                A(row, col) -= factor * A(zeroing_col,col);
                I(row, col) -= factor * I(zeroing_col,col);
            }
        }
    }
    Matrix* ret = new Matrix(N, N, 0);
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ret->M[i][j] = I(i, j);
        }
...
Matrix::Matrix(const Eigen::MatrixXd  & e) : row(e.rows()), col(e.cols()) {
    M = new double *[row];
...
Matrix* matrixFromArray(int row_size, int col_size, double** mat) {
    Matrix* m = new Matrix(row_size, col_size, 0);
    delete m->M;
    m->M = mat;
    return m;
}
...
const Matrix & Matrix::operator=(const Eigen::MatrixXd & rhs) {

    if ((row != rhs.rows()) || (col != rhs.cols())) {
        cout << "Miss-matched dimensions in = operator. " << row << " vs. " << rhs.rows() << " and " << col << " vs. " << rhs.cols() << endl;
        exit(1);
    }
