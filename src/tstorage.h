...
TStorageNode<K,D> * TStorage<K,D>::operator[](int index){
	if(mCursorPos < index)
		for(mCursorPos; mCursorPos < index; mCursorPos++)
			mCursor = mCursor->GetNext();
	else
		for(mCursorPos; mCursorPos < index; mCursorPos++)
			mCursor = mCursor->GetPrev();
	return mCursor;
}
